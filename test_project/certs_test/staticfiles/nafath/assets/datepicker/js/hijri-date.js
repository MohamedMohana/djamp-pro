...
function HijriDate() {
  ...
  if (isNaN(time)) time = Date.now();
    ...
    time = HijriDate.int(arguments[0], Date.now());
  else time += tzom;
  updDt(utc, time);
  updDt(loc, time - tzom);
  ...
  hd.toDateString = function () {
    let h = HijriDate,
      d = h.toNDigit;
    return (
      h.weekdayShortNames[loc.day] +
      " " +
      h.monthShortNames[loc.mmm] +
      " " +
      d(loc.ddd, 2) +
      " " +
      d(loc.yyy, 4)
  ...
  hd.toUTCString = function () {
    let h = HijriDate,
      d = h.toNDigit;
    return (
      h.weekdayShortNames[utc.day] +
      ", " +
      d(utc.ddd, 2) +
      " " +
      h.monthShortNames[utc.mmm] +
      " " +
      d(utc.yyy, 4) +
      " " +
      getUTCTmStr() +
